...
export const YZelements = ({toggleB = false, audio}) => {
  ...
  const animatedRingStyle = useSpring({
    // we do *not* animating this property, we just set it up
    strokeDasharray: length,
    strokeDashoffset: toggle ? 0 : length,
    strokeWidth: 2,
  ...
  const animatedFillStyle = useSpring({
    // we do *not* animating this property, we just set it up
    fill: toggle ? "url(#gradient) var(--sub-bg-color)" : "var(--sub-bg-color)",
    opacity: toggleB ? 1 : 0,
    strokeDasharray: 0,
    strokeDashoffset: 0,
    strokeWidth: 0,
  ...
  const animatedRingFillStyle = useSpring({
    // we do *not* animating this property, we just set it up
    fill: toggle ? "url(#gradient2) var(--sub-bg-color)" : "var(--sub-bg-color)",
    strokeDasharray: 0,
    strokeDashoffset: 0,
    strokeWidth: 0,
    delay: 500,
  ...
  //     setAudioState({ audioData: dataArray });
  ...
  return (
    ...
    >
      ...
      >
        <SplitterTop
          animatedStyle={animatedRingStyle}
          iref={(ref) => {
            if (ref) {
              setLength(ref.getTotalLength());
            }
